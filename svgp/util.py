...
class Welford(object):
    ...
    def update(self, new_value, batch=True):
        ...
        self._mean = new_value.new(*list(new_value.size())[1:]).zero_() if self._mean is None else self._mean
        self._sum_sq = new_value.new(*list(new_value.size())[1:]).zero_() if self._sum_sq is None else self._sum_sq
